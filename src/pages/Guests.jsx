...
const Guests = () => {
  ...
  const fetchGuests = async () => {
    try {
      ...
      if (response.data.success) {
        ...
        setTotalGuests(response.data.pagination?.total ?? response.data.data.length);
        setTotalPages(response.data.pagination?.totalPages ?? 1);
        setSummary(response.data.summary || { total: 0, male: 0, female: 0 });
      }
    } catch (err) {
  ...
  const openEditModal = (guest) => {
    ...
    setFormData({
      ...
      email: guest.email || '',
      phone: guest.phone || '',
      identity_type: guest.identity_type || 'identity_card',
    ...
    try {
      if (isEditMode && selectedGuest) {
        ...
        if (response.data.success) {
          ...
          setFormError("Berhasil menambah data");
        }
      }
    } catch (err) {
  ...
  const getIdentityLabel = (type) => (type === 'passport' ? 'Passport' : 'KTP');
      ...
      <div className="bg-white rounded-lg shadow-md p-6 mb-6">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="md:col-span-2">
            <label className="block text-sm font-medium text-gray-700 mb-2">Cari Tamu</label>
            <div className="relative">
              ...
              <input
                type="text"
                name="search"
                placeholder="Cari nama, email, nomor telepon, atau nomor identitas..."
        ...
        <div className="bg-gradient-to-br from-blue-500 to-blue-600 rounded-lg p-6 text-white shadow-md">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-3xl font-bold mt-1">{summary.male}</p>
            </div>
        ...
        <div className="bg-gradient-to-br from-pink-500 to-pink-600 rounded-lg p-6 text-white shadow-md">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-3xl font-bold mt-1">{summary.female}</p>
            </div>
            <span className="text-5xl">👩</span>
          </div>
        </div>
                ...
                guests.map((guest, index) => (
                  <tr key={guest.id} className="hover:bg-yellow-50 transition-colors">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="flex items-center">
                        ...
                        <div className="ml-4">
                          ...
                          <div className="text-xs text-gray-500">{guest.gender === 'male' ? 'Laki-laki' : 'Perempuan'}</div>
                        </div>
                      </div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-600">
                      ...
                      <div className="text-xs text-gray-400">{getIdentityLabel(guest.identity_type)}</div>
                    </td>
      ...
      {deleteConfirm && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg shadow-xl max-w-md w-full p-6">
            <h3 className="text-lg font-bold text-gray-800 mb-2">Hapus Data Tamu?</h3>
            <p className="text-gray-600 mb-6">
              Yakin ingin menghapus data <span className="font-semibold">{deleteConfirm.name}</span>? Tindakan ini tidak bisa dibatalkan.
            </p>
